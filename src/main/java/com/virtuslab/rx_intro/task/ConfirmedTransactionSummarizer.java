...
class ConfirmedTransactionSummarizer {
    ...
    Single<BigDecimal> summarizeConfirmedTransactions() {
        return zip(transactions.get(),confirmations.get(), (key, value) -> new Pair<Transaction, Confirmation>(key, value))
                .filter( x -> x.value.isConfirmed).map( transactionConfirmationPair -> transactionConfirmationPair.key.value)
                .reduce(BigDecimal::add)
                .onErrorResumeNext(throwable -> {
                    Observable.error(new SummarizationException(throwable.toString()));
                } )
                .toSingle();
    }
